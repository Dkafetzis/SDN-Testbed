...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def _save_to_file(self):
        if self.port_speed != {}:
            speeds = {3:{1:self.port_speed[(3,1)][-1],2:self.port_speed[(3,2)][-1]},4:{1:self.port_speed[(4,1)][-1],2:self.port_speed[(4,2)][-1]}}
        else:
            speeds = {3:{1:0,2:0},4:{1:0,2:0}}  
        if self.free_bandwidth != {}:
            bw = self.free_bandwidth
        else:
            bw = {3:{1:0,2:0},4:{1:0,2:0}} 
        results = {"switches_free_bw":bw, "switches_port_speeds":speeds}
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in [flow for flow in body if flow.priority != 65535][:4]:
            
            if stat.match['eth_type'] == 2048:
                key = (stat.match['eth_type'],stat.match['ipv4_src'],stat.instructions[0].actions[0].port)
            else:
                key = (stat.match['eth_type'],stat.match['arp_spa'],stat.instructions[0].actions[0].port)
